...
const Deposit = () => {
  ...
  const confirmDeposit = async () => {
    ...
    setLoading(true);
    try {
        const req = await fetch(`${process.env.NEXT_PUBLIC_DOMAIN}/api/admin/transactions/deposit`, {
            method: 'POST',
            headers: {
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-white dark:bg-gray-800 shadow-xl rounded-lg p-8 flex flex-col gap-6 w-full md:flex-row md:items-center md:justify-between max-w-3xl mx-auto">
        ...
        <form className="flex flex-col gap-4 w-full md:flex-row md:gap-6" onSubmit={handleDeposit}>
          <input
